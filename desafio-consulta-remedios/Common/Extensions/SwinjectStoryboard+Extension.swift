...
extension SwinjectStoryboard {
    @objc class func setup() {
        ...
        defaultContainer.register(GameRepositoryProtocol.self) { resolve in
            GameRepository(provider: resolve.resolve(MoyaProvider<GameService>.self)!)
        }
        defaultContainer.register(CartSingleton.self) { _ in CartSingleton.shared }
        defaultContainer.register(CartRepositoryProtocol.self) { resolve in
            CartRepository(cart: resolve.resolve(CartSingleton.self)!)
        }
        defaultContainer.storyboardInitCompleted(GamesController.self) { resolver, controller in
            let presenter = GamesPresenter(
                view: controller.self,
                router: GamesPresenterRouter(viewController: controller.self))
        ...
        defaultContainer.storyboardInitCompleted(GameDetailsController.self) { resolver, controller in
            let presenter = GameDetailsPresenter(
                view: controller.self,
                router: GameDetailsPresenterRouter(viewController: controller.self))
            presenter.gamesRepository = resolver.resolve(GameRepositoryProtocol.self)
            controller.presenter = presenter
        }
        defaultContainer.storyboardInitCompleted(ShoppingCartController.self) { resolver, controller in
            let presenter = ShoppingCartPresenter(
                view: controller.self,
                router: ShoppingCartPresenterRouter(viewController: controller.self))
